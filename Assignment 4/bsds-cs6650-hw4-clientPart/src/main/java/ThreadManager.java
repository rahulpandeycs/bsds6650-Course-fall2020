...
public class ThreadManager implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      sb.append("Number of failed requests: " + this.getGlobalCountFail().getCounter() + "\n");

      System.out.println("Number of Successful requests: " + this.getGlobalCountSuccess().getCounter());
      sb.append("Number of Successful requests: " + this.getGlobalCountSuccess().getCounter() + "\n");

      System.out.println("Mean responseTime:" + performanceMetrics.getMeanResponseTime() + " ms");
      sb.append("Mean responseTime:" + performanceMetrics.getMeanResponseTime() + " ms" + "\n");

      System.out.println("Median responseTime:" + performanceMetrics.getMedianResponseTime() + " ms");
      sb.append("Median responseTime:" + performanceMetrics.getMedianResponseTime() + " ms" + "\n");

      System.out.println("The total run time (wall time) :" + (endTime - startTime) + " ms");
      sb.append("The total run time (wall time) :" + (endTime - startTime) + " ms" + "\n");

      System.out.println("Total Requests: " + performanceMetrics.getTotalRequests());
      sb.append("Total Requests: " + performanceMetrics.getTotalRequests() + "\n");

      System.out.println("Throughput: " + (totalRequests * 1000 / (endTime - startTime)) + " requests/sec");
      sb.append("Throughput: " + (totalRequests * 1000 / (endTime - startTime)) + " requests/sec" + "\n");

      System.out.println("p99 (99th percentile) response time :" + performanceMetrics.getPercentile(99) + " ms");
      sb.append("p99 (99th percentile) response time :" + performanceMetrics.getPercentile(99) + "\n");

      System.out.println("Max response time:" + performanceMetrics.getMaxResponse() + " ms");
      sb.append("Max response time:" + performanceMetrics.getMaxResponse() + " ms" + "\n");

      File csvFile = new File("Metrics.txt");
